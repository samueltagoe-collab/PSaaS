...
import {
  ...
  BadgeCheck, ExternalLink, ShieldQuestion
} from "lucide-react";
...
const SEED_BUILDERS = {
  "kwame asante": { name: "Kwame Asante", portfolio: "Built two mapping and logistics apps for transport unions in Kumasi.", link: "" },
  "efua owusu": { name: "Efua Owusu", portfolio: "Ships offline-first POS and ledger tools for market traders; 3 shipped projects.", link: "" },
};
...
function useBuilders() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get("builders-directory", true);
        setBuilders(res && res.value ? JSON.parse(res.value) : SEED_BUILDERS);
      } catch {
        setBuilders(SEED_BUILDERS);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const saveBuilder = useCallback(async (entry) => {
    setBuilders((prev) => {
      const next = { ...prev, [entry.name.toLowerCase()]: entry };
      window.storage.set("builders-directory", JSON.stringify(next), true).catch(() => {});
      return next;
    });
  }, []);

  return { builders, saveBuilder, loading };
}
...
function VettingGate({ user, onVetted }) {
  const [portfolio, setPortfolio] = useState("");
  const [link, setLink] = useState("");

  return (
    <div className="bg-[#0B1830] border border-amber-400/30 rounded-sm p-4">
      <div className="flex items-center gap-2 text-amber-300 mb-2">
        <ShieldQuestion size={16} />
        <span className="text-sm font-medium">Before you propose, build your builder profile</span>
      </div>
      <p className="text-xs text-slate-400 mb-3">Owners want to know who they're trusting with their idea. This shows on every proposal you send — you only fill it in once.</p>
      <textarea value={portfolio} onChange={(e) => setPortfolio(e.target.value)} rows={2}
        placeholder="What have you built before? e.g. 'Shipped 3 fintech apps, 2 years freelance backend work.'"
        className="w-full bg-[#0F1F38] border border-slate-700 rounded-sm px-3 py-2 text-sm text-slate-100 mb-2 focus:outline-none focus:border-cyan-400" />
      <input value={link} onChange={(e) => setLink(e.target.value)} placeholder="Link to your work (optional) — GitHub, portfolio, LinkedIn"
        className="w-full bg-[#0F1F38] border border-slate-700 rounded-sm px-3 py-2 text-sm text-slate-100 mb-3 focus:outline-none focus:border-cyan-400" />
      <button
        disabled={!portfolio.trim()}
        onClick={() => onVetted({ name: user.name, portfolio: portfolio.trim(), link: link.trim() })}
        className="bg-amber-400 disabled:bg-slate-700 disabled:text-slate-500 text-[#0B1830] font-semibold rounded-sm px-4 py-2 text-sm hover:bg-amber-300"
      >
...
function IdeaDetail({ idea, user, builders, onSaveBuilder, onBack, onUpdate }) {
  ...
  const isVetted = Boolean(builders[user.name.toLowerCase()]);
          ...
          <div className="border-t border-cyan-900/50 pt-5">
            ...
            <div className="space-y-3 mb-4">
              {idea.proposals.map((p) => {
                const bp = builders[p.proName.toLowerCase()];
                return (
                <div key={p.id} className="bg-[#0B1830] border border-slate-700 rounded-sm p-3">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-slate-100 font-medium text-sm flex items-center gap-1.5">
                      {p.proName}
                      {bp ? (
                        <span className="flex items-center gap-0.5 text-[10px] text-emerald-300 font-mono font-normal"><BadgeCheck size={12} />Vetted</span>
                      ) : (
                        <span className="text-[10px] text-slate-500 font-mono font-normal">Unverified</span>
                      )}
                    </span>
                  ...
                  {bp && (
                    <p className="text-xs text-slate-500 mb-1.5 italic">
                      {bp.portfolio}
                      {bp.link && <a href={bp.link} target="_blank" rel="noreferrer" className="text-cyan-400 ml-1 inline-flex items-center gap-0.5 not-italic">view<ExternalLink size={10} /></a>}
                    </p>
                  )}
            ...
            {!isOwner && !hasProposed && !isVetted && (
              <VettingGate user={user} onVetted={(entry) => onSaveBuilder(entry)} />
            )}
            {!isOwner && !hasProposed && isVetted && (
...
export default function App() {
  ...
  const { builders, saveBuilder } = useBuilders();
    ...
    >
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        {loading ? (
          ...
          <IdeaDetail idea={active} user={user} builders={builders} onSaveBuilder={saveBuilder} onBack={() => setView("board")} onUpdate={updateIdea} />
        ) : (
          <div>
            <div className="mb-6">
